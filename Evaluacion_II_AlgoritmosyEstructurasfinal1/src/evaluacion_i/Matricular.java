...
public class Matricular extends JFrame implements ActionListener {
    ...
    public void leermatriz(int matriz[][], int i, int j) {
        if (i < matriz.length) {
            if (j < matriz.length) {
                ...
                leermatriz(matriz, i, j + 1);
            } else {
                areatext.append("\n");
                System.out.println("");

                leermatriz(matriz, i + 1, 0);
            }
        }
    }

    public void leerfibonacci() {
        if (c <= propos) {
            fibonacci.append(c + ",");
            a = b;
            b = c;
